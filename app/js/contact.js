$(document).ready(function() {
    ...
    $('button[type="submit"]').click(function(event) {
        if ($(".not-error").length == 3) {
			...
			$('.send-name, .contact-text').qtip('toggle', true);
            console.log("errore");
        }
    });
    $('input, textarea').placeholder();
    $('button[type="reset"]').click(function(){ $('.send-name, .contact-text').removeClass('error');
	$('.send-name, .contact-text').qtip('destroy', true); 		   
	});
});